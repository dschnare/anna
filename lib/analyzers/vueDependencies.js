...
module.exports = function vueDependencies ({ cssImportPaths = [], jsDependencyOptions = {} } = {}) {
  const _jsDependencies = jsDependencies(
    Object.assign({ '.vue': getScriptDependencies }, jsDependencyOptions)
  )
  const _cssDependencies = cssDependencies({ paths: cssImportPaths })

  function getScriptDependencies (sourceFile, sourceText, anna) {
  ...
  return (sourceFile, sourceText, anna) => {
    return Promise.all([
      getScriptDependencies(sourceFile, sourceText, anna),
      (function () {
