...
const vueDependencies = require('./vueDependencies')
...
function getDependencies (sourceFile, sourceText) {
  ...
  return new Promise((resolve, reject) => {
    try {
      deps = deps.map(dep => {
        return dir.charAt(0) === '.' ? path.resolve(dir, dep) : dep
      }).map(dep => {
        try {
...
module.exports = function jsDependencies (options = {}) {
  return makeRecursiveDependencyAnalyzer(getDependencies, Object.assign({
    '.vue': vueDependencies()
  }, options || {}))
}
